...
import { getBrandCardoc } from './getBrandCardoc';
...
export default async function generateBrandStats(owner: string, repo: string) {
  ...
  const cacheStarData = brandCache.get(`${owner}/${repo}`);
  if (cacheStarData) {
    ...
    try {
      ...
      brandCache.set(`${owner}/${repo}`, stargazerDataFull);
    } catch (e) {
