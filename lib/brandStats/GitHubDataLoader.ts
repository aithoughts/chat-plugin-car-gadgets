  ...
  public async fetchStargazers(user: string, repo: string): Promise<any | null> {
    try {
      ...
      if (
        ...
        throw Error(
          `Cannot load a repo with more than ${100 * MAX_PAGES_WITHOUT_TOKEN} stars without CarMake access token. Please click "CarMake Authentication" and provide one`
        );
      }
      ...
      if (error.response.status === 404) {
        throw Error(`Brand ${user}/${repo} Not found`);
      } else if (error.response.status === 403) {
        throw Error(
          'API rate limit exceeded! Please click "CarMake Authentication" and provide CarMake access token to increase rate limit'
        );
      } else {
        throw Error(
